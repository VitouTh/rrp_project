        ...
        <div class="card-header border-0 py-5">
          ...
          <div class="card-toolbar">
            <b-button
              ...
              v-on:click.prevent="$router.push('meeting/form')"
            >
              ...
              >
                ...
                <template #cell(action)="data">
                  <a
                    href="javascript:"
                    @click.prevent="
                      $router.push(`meeting/form/${data.item.id}`)
                    "
...
import {
  PAGINATION_MEETING_DATA,
  SET_PAGINATION,
  SET_IS_CLEAR_PAGINATION,
  CLEAR_PAGINATION,
  PUT_MEETING_STATUS_DATA,
  DELETE_MEETING_DATA
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "customer_name",
          label: `${this.$t("PAGE.MEETING.TABLE.FIELDS.CUSTOMER")}`,
          key_table: "customer_name",
          sortable: false,
          isFilter: false,
          tdClass: "nameField"
        },
        {
          key: "customer_num",
          label: `${this.$t("PAGE.MEETING.TABLE.FIELDS.PHONE_NUMBER")}`,
          key_table: "customer_num",
          sortable: false,
          isFilter: false,
          tdClass: "numField"
        },
        {
          key: "admin_name",
          label: `${this.$t("PAGE.MEETING.TABLE.FIELDS.ADMIN")}`,
          key_table: "admin_name",
          sortable: false,
          isFilter: false,
          tdClass: "nameField"
        },
        {
          key: "datetime",
          label: `${this.$t("PAGE.MEETING.TABLE.FIELDS.DATETIME")}`,
          key_table: "datetime",
          sortable: true,
          isFilter: true,
          tdClass: "datetimeField"
        },
        {
          key: "switch",
      ...
      select: ["meeting.id", "meeting.datetime","meeting.status_id"],
  ...
  computed: {
    ...
    isClearPagination: {
      get() {
        return this.$store.state.meeting.isClearPagination;
      },
      set(value) {
    ...
    pagination: {
      get() {
        return this.$store.state.meeting.pagination;
      },
      set(value) {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      { title: `${this.$t("MENU.MEETING.TITLE")}` }
    ]);
  ...
  methods: {
    ...
    async fetchData() {
      await this.$store
        .dispatch(PAGINATION_MEETING_DATA, { params: this.getParams() })
        .then(({ data }) => {
          
          if (data.status === "success" && data.data) {
            this.items = data.data.data;
            for(let i in data.data.data){
              for(let t in this.adminData){
                if(data.data.data[i]['admin_id'] == this.adminData[t]['id']){
                  this.items[i].admin_name = this.adminData[t].fullname
                }
              }
              for(let t in this.customerData){
                if(data.data.data[i]['customer_id'] == this.customerData[t]['id']){
                  this.items[i].customer_num =  this.customerData[t].phone_number
              this.items[i].customer_name =  this.customerData[t].fullname
                }
              }
    ...
    updateStatus(event, id) {
      this.isBusy = true;
      this.$store
        .dispatch(PUT_MEETING_STATUS_DATA, {
          id,
          data: {
    ...
    removeData(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store
            .dispatch(DELETE_MEETING_DATA, { id })
            .then(({ data }) => {
              if (data.status === "success") {
                Swal.fire({
                  icon: "success",
        ...
        preview(id) {
      window.open(`${Config.PREVIEW_CONTENT_URL}/meeting?id=${id}`);
    }
  }
};
</script>
