        ...
        <div class="card-header border-0 py-5">
          ...
          <div class="card-toolbar">
            <b-button
              ...
              v-on:click.prevent="$router.push('product_type/form')"
            >
                  ...
                  <div class="d-flex align-items-center ">
                    <div class="one-line-clamp">
                      <a
                        href="javascript:"
                        @click.prevent="
                          $router.push(`product_type/form/${data.item.id}`)
                        "
...
import {
  PAGINATION_PRODUCT_TYPE_DATA,
  SET_PAGINATION,
  SET_IS_CLEAR_PAGINATION,
  CLEAR_PAGINATION,
  PUT_PRODUCT_TYPE_STATUS_DATA,
  DELETE_PRODUCT_TYPE_DATA
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "name_en",
          label: `${this.$t("PAGE.PRODUCT_TYPE.TABLE.FIELDS.NAME_EN")}`,
          key_table: "product_type.name_en",
          sortable: true,
          isFilter: true,
          tdClass: "nameEnField"
        },
        {
          key: "name_kh",
          label: `${this.$t("PAGE.PRODUCT_TYPE.TABLE.FIELDS.NAME_KH")}`,
          key_table: "product_type.name_kh",
          sortable: true,
          isFilter: true,
          tdClass: "nameKhField"
        },
      ...
      select: ["product_type.id", "product_type.name_en", "product_type.name_kh","product_type.status_id"],
  ...
  computed: {
    ...
    isClearPagination: {
      get() {
        return this.$store.state.productType.isClearPagination;
      },
      set(value) {
    ...
    pagination: {
      get() {
        return this.$store.state.productType.pagination;
      },
      set(value) {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      { title: `${this.$t("MENU.PRODUCT_TYPE.TITLE")}` }
    ]);
  ...
  methods: {
    ...
    fetchData() {
      this.$store
        .dispatch(PAGINATION_PRODUCT_TYPE_DATA, { params: this.getParams() })
        .then(({ data }) => {
          console.log(data)
    ...
    updateStatus(event, id) {
      this.isBusy = true;
      this.$store
        .dispatch(PUT_PRODUCT_TYPE_STATUS_DATA, {
          id,
          data: {
    ...
    removeData(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store
            .dispatch(DELETE_PRODUCT_TYPE_DATA, { id })
            .then(({ data }) => {
              if (data.status === "success") {
                Swal.fire({
                  icon: "success",
        ...
        preview(id) {
      window.open(`${Config.PREVIEW_CONTENT_URL}/product_type?id=${id}`);
    }
  }
};
</script>
