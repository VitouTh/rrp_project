        ...
        <div class="card-header border-0 py-5">
          ...
          <div class="card-toolbar">
            <b-button
              ...
              v-on:click.prevent="$router.push('blog/form')"
            >
                  ...
                  <div class="d-flex align-items-center ">
                    <div class="one-line-clamp">
                      <a
                        href="javascript:"
                        @click.prevent="
                          $router.push(`blog/form/${data.item.id}`)
                        "
...
import {
  PAGINATION_BLOG_DATA,
  SET_PAGINATION,
  SET_IS_CLEAR_PAGINATION,
  CLEAR_PAGINATION,
  PUT_BLOG_STATUS_DATA,
  DELETE_BLOG_DATA
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "title",
          label: `${this.$t("PAGE.BLOG.TABLE.FIELDS.TITLE")}`,
          key_table: "blog.title",
          sortable: true,
          isFilter: true,
          tdClass: "titleField"
        },
        {
          key: "subtitle",
          label: `${this.$t("PAGE.BLOG.TABLE.FIELDS.SUBTITLE")}`,
          key_table: "blog.subtitle",
          sortable: true,
          isFilter: true,
          tdClass: "subtitleField"
        },
        {
          key: "switch",
      ...
      select: ["blog.id", "blog.title", "blog.subtitle","blog.status_id"],
  ...
  computed: {
    ...
    isClearPagination: {
      get() {
        return this.$store.state.blog.isClearPagination;
      },
      set(value) {
  ...
  methods: {
    ...
    fetchData() {
      this.$store
        .dispatch(PAGINATION_BLOG_DATA, { params: this.getParams() })
        .then(({ data }) => {
          console.log(data)
    ...
    removeData(id) {
      Swal.fire({
        ...
        preview(id) {
      window.open(`${Config.PREVIEW_CONTENT_URL}/blog?id=${id}`);
    }
  }
};
</script>
