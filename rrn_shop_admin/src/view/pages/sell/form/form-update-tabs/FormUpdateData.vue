              ...
              <b-col sm="10"><table class="table">
                ...
                <tbody>
                  <tr v-for="(item, index) in form.productSelect" :value="item.id" :key="item">
                    ...
                    href="javascript:"
                    @click.prevent="
                      updateProductData(item.product_id, index)
                    "
                  ...
                  ></td> -->
                    <td> <a
                    href="javascript:"
                    @click.prevent="removeProductData(item.product_id, index)"
                    class="btn btn-icon btn-light btn-sm"
                    v-b-tooltip.hover
...
import {
  GET_SELL_DATA,
  PUT_SELL_DATA
} from "@/core/services/store/sell.module";
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      for(let i in this.form.productSelect){
        ...
        if(this.form.productSelect[i].name != "" && this.form.productSelect[i].quantity != ""){
          let index = this.productData.findIndex((obj) => obj.id == this.form.productSelect[i].product_id);
          console.log(this.form.productSelect[i])
          ...
          this.$store
            ...
            .then(({ data }) => {
              ...
              if (data.status === "success" && data.data) {
                ...
                this.form.productSelect = data.data.sell_product
                
                for(let i in data.data.sell_product){
                  const index = this.productData.map(object => object.id).indexOf(data.data.sell_product[i].product_id);
                  data.data.sell_product[i].name = this.productData[index].product
    ...
    update(id, data) {
      Swal.fire({
        ...
        onOpen: () => {
          Swal.showLoading();
          this.$store
            .dispatch(PUT_SELL_DATA, { id, data })
            .then(({ data }) => {
              Swal.close();
              if (data.status === "success" && data.data) {
                Swal.fire({
    ...
    editProductConfirm(){
      let index = this.productData.findIndex((obj) => obj.id == this.product);
      let product_index = this.form.productSelect.findIndex((obj) => obj.productSelect == this.product);
      if(this.product == "" || this.quantity == "" || this.quantity < 1){
        Swal.fire({
            icon: "error",
            text: `Please input data`,
      ...
      if(product_index == this.editIndex){
        if(this.productData[index].stock + parseInt(this.form.productSelect[this.editIndex].quantity)<  this.quantity){
        Swal.fire({
            icon: "error",
            text: `Stock is not enough`,
            confirmButtonText: `${this.$t("PAGE.DEFAULT.FORM.BTN_OK")}`
      ...
      this.form.productSelect[this.editIndex].product_id = this.product;
      this.productData[index].selected = true;
      this.product = "";
      this.quantity = "";
