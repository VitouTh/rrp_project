        ...
        <div class="card-header border-0 py-5">
          ...
          <div class="card-toolbar">
            <b-button
              ...
              v-on:click.prevent="$router.push('poster/form')"
            >
                  ...
                  <div class="d-flex align-items-center ">
                    <div class="one-line-clamp">
                      <a
                        href="javascript:"
                        @click.prevent="
                          $router.push(`poster/form/${data.item.id}`)
                        "
...
import {
  PAGINATION_POSTER_DATA,
  SET_PAGINATION,
  SET_IS_CLEAR_PAGINATION,
  CLEAR_PAGINATION,
  PUT_POSTER_STATUS_DATA,
  DELETE_POSTER_DATA
} from "@/core/services/store/poster.module";
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "description",
          label: `${this.$t("PAGE.POSTER.TABLE.FIELDS.DESCRIPTION")}`,
          key_table: "poster.description",
          sortable: true,
          isFilter: true,
          tdClass: "descriptionField"
        },
        {
          key: "switch",
      ...
      select: ["poster.id", "poster.description","poster.status_id"],
  ...
  computed: {
    ...
    isClearPagination: {
      get() {
        return this.$store.state.poster.isClearPagination;
      },
      set(value) {
    ...
    pagination: {
      get() {
        return this.$store.state.poster.pagination;
      },
      set(value) {
  ...
  methods: {
    ...
    fetchData() {
      console.log(this.$store.dispatch(PAGINATION_POSTER_DATA, { params: this.getParams() }))
      this.$store
        .dispatch(PAGINATION_POSTER_DATA, { params: this.getParams() })
        .then(({ data }) => {
    ...
    updateStatus(event, id) {
      this.isBusy = true;
      this.$store
        .dispatch(PUT_POSTER_STATUS_DATA, {
          id,
          data: {
    ...
    removeData(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$store
            .dispatch(DELETE_POSTER_DATA, { id })
            .then(({ data }) => {
              if (data.status === "success") {
                Swal.fire({
                  icon: "success",
        ...
        preview(id) {
      window.open(`${Config.PREVIEW_CONTENT_URL}/poster?id=${id}`);
    }
  }
};
</script>
