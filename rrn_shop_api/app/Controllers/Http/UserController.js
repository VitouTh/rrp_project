...
class UserController extends BaseController {
            ...
            if (request.email && request.password) {
                ...
                if (!_.isEmpty(user)) {
                    if (await context.auth.attempt(request.email, request.password)) {
                        ...
                        if (parseInt(user.user_type_id) == 3) {
        ...
        try {
            ...
            if (!_.isEmpty(user)) {
                ...
                if (await user.save()) {
                    ...
                    userUserType.user_type_id = UserType.$ADMIN
    ...
    async getSell(userId) {
        let sell = await Sell.query()
            .where('user_id', userId)
            .where('status_id', Status.$ACTIVE)
            .fetch()
        ...
        try {
            ...
            if (queryParams) {
                ...
                datas.data = []
                    // console.log(data)
                for (let i in data) {
                    let user_type_id = await this.getUserUserTypeId(data[i].id)
                    if (user_type_id == 1 || user_type_id == 2) {
                        datas.data.push(data[i])
                    }
                }


                // // datas.new_data = new_data
    ...
    async paginationCustomer({ request, auth, response }) {
        try {
            ...
            if (queryParams) {
                ...
                for (let i in data) {
                    ...
                    if (user_type_id == 3) {
                        ...
                        let sell = await this.getSell(data[i].id)
                        sell = sell.toJSON()
                        if (sell.length == 0) {
                            total['total'] = 0
                        } else {
                            for (let t in sell) {
                                total['total'] = total['total'] + sell[t].total
                            }
                        }
                        data[i] = Object.assign(data[i], total);
                        datas.data.push(data[i])
                    }
                }
    ...
    async listCustomer({
        ...
        try {
            ...
            let user_type_id = 0
            for (let i in datas) {
                user_type_id = await this.getUserUserTypeId(datas[i].id)
                if (user_type_id == 3) {
